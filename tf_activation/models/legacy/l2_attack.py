...
class CarliniL2:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        self.newimg = tf.tanh(modifier + self.timg, name="tanh") * self.boxmul + self.boxplus
        ...
        var = [v for v in tf.global_variables() if v.name == "timg:0" or v.name == "tanh:0" or v.name=="modifier:0"]
        self.sess.run(tf.variables_initializer(var_list=var))
    ...
    def attack_batch(self, imgs, labs):
        ...
        # print(imgs[np.isnan(imgs)])
        imgs = np.arctanh((imgs - self.boxplus) / self.boxmul * 0.999999)
        # imgs[np.isnan(imgs)] = 1.0

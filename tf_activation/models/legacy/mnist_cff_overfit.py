...
def run(return_after=None):
    ...
    with tf.Session(config=config) as sess:
        ...
        print('saving ...')
        df = pd.DataFrame(df)
        save_path = saver.save(sess, os.path.join(SAVE_PATH, 'mnist_cff_' + str(return_after) + '_overfit_0.ckpt'))
        df.to_pickle(os.path.join(SAVE_PATH, 'mnist_cff_' + str(return_after) + '_overfit_0.pkl'))

...
H = 0
EPOCHS = 500
DIAG_DIR = '../../logdir/data/experiments/mnist_ccff50_' + str(EPOCHS) + '_' + str(p)
...
def deepnn(x):
   ...
   with tf.device('/cpu:0'):
       ...
       ret['h_conv2_reshaped'] = tf.reshape(ret['h_conv2'], [-1, 28*28*64])

       ret['W_fc1'] = weight_variable([28 * 28 * 64, 1024])
       ret['b_fc1'] = bias_variable([1024])

       ret['h_fc1'] = tf.nn.relu(tf.matmul(ret['h_conv2_reshaped'], ret['W_fc1']) + ret['b_fc1'])
